...
- to fix CORS issue:
  ...
  - 🚨 TODO: it doesn't work for now. So temporarily we have allowed all domains in cors settings in main.ts. This is not recommended for production
...
- TODO: Currently the jwt authorization token is not being passed to the server when making graphql call for getMe. Need to fix this before next stream
